<template>
  <el-dialog v-model="dialogShow" :title="role.name ? ('编辑 ' + role.name): '新增角色'" @close="$emit('close', {})">
    <el-form ref="formRef" :model="form" :rules="rules" label-position="right" label-width="80px">
      ...
      <el-form-item prop="routes" label="权限">
        <el-tree :data="getRouteTree()" :show-checkbox="true" node-key="path"
                 :default-checked-keys="role.routes ? role.routes : []"
                 @check="checkNodes">
          <template #default="{data}">
            <el-col v-if="!data.meta.public" :lg="12" :md="24">{{ data.meta.title }}: {{ data.name }}</el-col>
          </template>
        </el-tree>
      </el-form-item>
    </el-form>
    ...
    if (!route.meta.public && (route.meta.header)) {
      if (route.children) {
        route.children.forEach((item, idx) => {
          route.children[idx] = Object.assign(item, {disabled: !props.editable})
        })
      }
