  ...
  <el-container>
    ...
    <el-container :class="'page-content'  + ('Index' === $route.name ? ' homepage' : '')">
      <template v-if="$store.getters.isLogged">
        <el-button class="btn-router-menu opened"
                   circle plain
                   v-if="showMenu"
                   :icon="DArrowLeft"
                   @click="showMenu = false"/>
        <el-button class="btn-router-menu closed"
                   round plain
                   v-else
                   :icon="DArrowRight"
                   @click="showMenu=true"/>
      ...
      <el-main :class="'main-content' + (showMenu ? ' show-router-menu' : '')" v-loading="$store.getters.isLoading">
        ...
        <el-scrollbar class="page-main">
          <el-row v-if="isSubmenu(currentRoute)">
            <el-col :lg="6" :md="12" :sm="24" v-for="route in currentRoute.children" :key="route.path">
              <el-link @click="$router.push({name: route.name})" type="primary">{{ route.meta.title }}</el-link>
            </el-col>
          </el-row>
          <template v-else>
...
const initPageTabs = () => {
  if (!proxy.$store || !proxy.$store.getters || !proxy.$store.getters.isLogged) return
  tabs.value = []
    ...
    proxy.$route.matched.forEach(menu => {
      if (proxy.$route.name === menu.name && menu.children && menu.children.length > 0) {
        menu.children.forEach(tab => {
          if (tab.meta.tab && (proxy.$store.getters.userInfo.routes.indexOf(tab.path) >= 0 || tab.meta.public)) {
